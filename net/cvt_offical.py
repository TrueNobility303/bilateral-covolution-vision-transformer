...
class BiseCvt_same_params(nn.Module):
    def __init__(self,pth,hidden_dim=64):
        super().__init__()
        #num_classes=0时充当特征提取器的功能
        self.cvt = ConvolutionalVisionTransformer(
                 in_chans=3,
                 num_classes=0,
                 act_layer=nn.GELU,
                 ...
                 spec=CVT13_SPEC)
        self.cvt.init_weights(pth,['stage0','stage1','stage2'],True)
        self.wrn = WideResNet(depth=4,width_factor=6,dropout=0,in_channels=3,labels=hidden_dim)
        self.head = nn.Sequential(
            nn.Linear(384+hidden_dim, 2656),
            nn.ReLU(),
            nn.Linear(2656,10)) 

    def forward(self,x):
        x_big = F.interpolate(x, size=(224,224), mode='bilinear', align_corners=True)
        
        x_big = self.cvt(x_big)
        x_small = self.wrn(x)
...
class BiseCvt_same_flops(nn.Module):
    def __init__(self,pth,hidden_dim=64):
        super().__init__()
        #num_classes=0时充当特征提取器的功能
        self.cvt = ConvolutionalVisionTransformer(
                 ...
                 spec=CVT13_SPEC)
        self.cvt.init_weights(pth,['stage0','stage1','stage2'],True)
        self.wrn = WideResNet(depth=8,width_factor=2,dropout=0,in_channels=3,labels=hidden_dim)
        self.head = nn.Linear(384+hidden_dim, 10)
